...
const CommentForm: React.FC<CommentFormProps> = ({ id, product }) => {
  ...
  const [replyCommentFormSubmitting, setReplyCommentFormSubmitting] =
    useState(false);
  const [commentForm, setCommentForm] = useState<CommentDTO>({} as CommentDTO);
  const [replyCommentForm, setReplyCommentForm] = useState<CommentDTO>(
    {} as CommentDTO,
  );
  ...
  const handleCommentInputChange = (field: keyof CommentDTO, value: any) => {
    setCommentForm((prev) => ({
      ...prev,
      [field]: value || null,
    }));
  ...
  const onReplyCommentSubmit = async () => {
    try {
      ...
      replyCommentForm.parent_id = selectedComment!.id.toString();
      setReplyCommentFormSubmitting(true);

      const resp = await sendCommentProductById(Number(id), replyCommentForm);
      if (resp.success) {
        showMessage(resp.result);
        setReplyCommentForm({} as CommentDTO);
        closeDialogomments();
    ...
    <>
      ...
      {isOpenComments && (
        ...
        >
          ...
          >
            <h2>{t("local_commentReplyTo")} {selectedComment!.user_name}</h2>
              ...
              <div className="form-row textarea-field">
                ...
                >
                  ...
                  <Rate
                    //allowHalf
                    onChange={(value) =>
                      handleReplyCommentInputChange("rate", value)
                    }
                    value={replyCommentForm.rate || 0}
                    className="black-rate"
                  />
                </div>
              </div>
